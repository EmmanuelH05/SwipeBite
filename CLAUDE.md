...
**Test coverage:** `backend/src/lib/*.test.ts` covers `ml-recommender.ts` (the actual differentiator), `personalization.ts`, `prefHelpers.ts`, `auth.ts`, `rateLimit.ts`, `startupChecks.ts`, plus route-level pre-DB-validation branches for `photos.ts`, `swipes.ts`, `onboarding.ts`, `auth.ts`, and `app.ts` (the `/health` and 404/error-handler behavior). Not covered: anything needing a live DB (register/login/refresh flow, the swipe/preference-update transaction, the onboarding-seed transaction, restaurant loading/upserting) — the Supabase project was paused when these were written. **DB is reachable again as of 2026-08-11** (all 13 migrations applied and verified live, including two that had been hand-authored against an unreachable DB — see Branch History); the DB-dependent test suites are now unblocked but not yet written. Frontend has no test script yet — out of scope so far.
